# Smohix platform completion plan
...
- [x] **Lemon webhook** — signature verify, **SHA-256 idempotent** deliveries, subscription upsert when `meta.custom_data.smohix_user_id` (or `supabase_user_id`) is present.
- [x] **Deprecation follow-up** — legacy key fallback removed; checkout + webhooks use `smohix_user_id`.
...
- [x] **Marketing CTAs (signed-in)** — homepage `Header` / `Hero` / `ConnectCTA` use `getSignedInCheckoutUrl()` so trial links include `smohix_user_id` when auth + `NEXT_PUBLIC_LEMONSQUEEZY_CHECKOUT_URL` are set; anonymous visitors keep generic checkout or `#trial`.
...
- [x] **Settings → Billing** — `/settings/billing` shows plan, subscription snapshot, **Open checkout** (with `smohix_user_id`), and migration hints if the DB is not ready.
- [x] **Console nav** — **clickable module boxes** (`CONSOLE_MODULES` + `/hub` home); **Settings** hub, **Billing**, **API keys**, and **Connectors** included.
- [x] **Copilot without upstream** — `POST /api/copilot/chat` uses **OpenAI** when `OPENAI_API_KEY` is set, else **offline** replies; default chat path no longer requires `SMOHIX_REASONING_API_URL`.
- [x] **API keys without Supabase** — **session-scoped keys** in server memory (per `smohix_dev_tid` cookie) + proxy validation for `/api/reasoning` and `/api/robot` when auth env is off; Postgres-backed keys when auth is on.
...
- **Explicit checkout custom data** — without `smohix_user_id`, subscription webhooks return **422** and release the delivery idempotency slot so Lemon can retry after you fix checkout.
