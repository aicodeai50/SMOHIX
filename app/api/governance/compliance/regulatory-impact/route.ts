...
import {
  buildRegulatoryChangeImpactPack,
  regulatoryChangeImpactToCsv,
...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildRegulatoryChangeImpactPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build regulatory change impact report." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.regulatory_impact_exported",
    user_id: user.id,
    ...
    details: {
      ...
      highest_impact: pack.highestImpactScenarioId,
    },
  });
  ...
  if (format === "csv") {
    return new NextResponse(regulatoryChangeImpactToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="regulatory-impact-${stamp}.csv"`,
      },
    });
  }
  ...
  return NextResponse.json(pack, {
    ...
    headers: {
      ...OPERATIONAL_RESPONSE_HEADERS,
      "Content-Disposition": `attachment; filename="regulatory-impact-${stamp}.json"`,
    },
  });
}
