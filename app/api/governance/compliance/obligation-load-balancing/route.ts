...
import {
  buildObligationOwnerLoadBalancingPack,
  obligationOwnerLoadBalancingToCsv,
...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildObligationOwnerLoadBalancingPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build obligation owner load balancing pack." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.obligation_owner_load_balancing_exported",
    user_id: user.id,
    ...
    details: {
      ...
      peak_count: pack.peakWeekObligationCount,
      suggestion_count: pack.suggestions.length,
      imbalance_score: pack.imbalanceScore,
    },
  });
  ...
  if (format === "csv") {
    return new NextResponse(obligationOwnerLoadBalancingToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="obligation-load-balancing-${stamp}.csv"`,
      },
    });
  }
