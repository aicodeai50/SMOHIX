...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildStaffingSlaBreachDigestPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build staffing SLA breach digest." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.staffing_sla_breach_digest_exported",
    user_id: user.id,
    org_id: orgContext.orgId,
    details: {
      format,
      breach_count: pack.breachItems.length,
      sla_days: pack.settings.slaDaysAfterPeakWeek,
  ...
  if (format === "csv") {
    return new NextResponse(staffingSlaBreachDigestToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="staffing-sla-breach-${stamp}.csv"`,
      },
    });
  }
