...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildPeakWeekStaffingDigestPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build peak-week staffing digest." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.peak_week_staffing_digest_exported",
    user_id: user.id,
    org_id: orgContext.orgId,
    details: {
      format,
      should_alert: pack.coincidence.shouldAlert,
      peak_week_key: pack.coincidence.peakWeekKey,
  ...
  if (format === "csv") {
    return new NextResponse(peakWeekStaffingDigestToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="peak-week-staffing-${stamp}.csv"`,
      },
    });
  }
