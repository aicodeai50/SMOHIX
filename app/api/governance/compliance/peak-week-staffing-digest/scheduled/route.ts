...
/** Cron: POST with Bearer SMOHIX_PEAK_WEEK_STAFFING_DIGEST_CRON_SECRET. Body: `{ orgId, force? }` */
export async function POST(req: NextRequest) {
  const secret = (process.env.SMOHIX_PEAK_WEEK_STAFFING_DIGEST_CRON_SECRET ?? process.env.ZENTRO_PEAK_WEEK_STAFFING_DIGEST_CRON_SECRET)?.trim();
  if (!secret) {
