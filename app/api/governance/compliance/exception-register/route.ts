...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildComplianceExceptionRegisterPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build compliance exception register." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.compliance_exception_register_exported",
    user_id: user.id,
    ...
    details: {
      ...
      open: pack.openCount,
      approved: pack.approvedCount,
      expired: pack.expiredCount,
    },
  });
  ...
  if (format === "csv") {
    return new NextResponse(complianceExceptionRegisterToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="exception-register-${stamp}.csv"`,
      },
    });
  }
  ...
  return NextResponse.json(pack, {
    ...
    headers: {
      ...OPERATIONAL_RESPONSE_HEADERS,
      "Content-Disposition": `attachment; filename="exception-register-${stamp}.json"`,
    },
  });
}
