...
/** Cron: POST with Bearer SMOHIX_EVIDENCE_REQUEST_SLA_CRON_SECRET. Body: `{ orgId }` */
export async function POST(req: NextRequest) {
  const secret = (process.env.SMOHIX_EVIDENCE_REQUEST_SLA_CRON_SECRET ?? process.env.ZENTRO_EVIDENCE_REQUEST_SLA_CRON_SECRET)?.trim();
  if (!secret) {
  ...
  const result = await deliverEvidenceRequestSlaDigest(actorId, orgId, {
