...
/** Cron: POST with Bearer SMOHIX_MAPPING_DIGEST_CRON_SECRET. Body: { orgId } */
export async function POST(req: NextRequest) {
  const secret = (process.env.SMOHIX_MAPPING_DIGEST_CRON_SECRET ?? process.env.ZENTRO_MAPPING_DIGEST_CRON_SECRET)?.trim();
  if (!secret) {
  ...
  const result = await runRegulatoryMappingDigestForOrg(actorId, orgId, {
