...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildComplianceControlHealthScorecardPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build compliance control health scorecard." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.compliance_control_health_scorecard_exported",
    user_id: user.id,
    org_id: orgContext.orgId,
    details: {
      format,
      period_days: pack.periodDays,
      health_score: pack.healthScore,
      grade: pack.grade,
      posture_score: pack.postureScore,
      vendor_health_score: pack.vendorHealthScore,
  ...
  if (format === "csv") {
    return new NextResponse(complianceControlHealthScorecardToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="control-health-scorecard-${stamp}.csv"`,
      },
    });
  }
  ...
  return NextResponse.json(pack, {
    ...
    headers: {
      ...OPERATIONAL_RESPONSE_HEADERS,
      "Content-Disposition": `attachment; filename="control-health-scorecard-${stamp}.json"`,
    },
  });
}
