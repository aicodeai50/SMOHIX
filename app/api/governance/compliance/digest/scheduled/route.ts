...
/** Cron entrypoint: POST with `Authorization: Bearer <SMOHIX_DIGEST_CRON_SECRET>`. Body: `{ orgId, periodDays? }` */
export async function POST(req: NextRequest) {
  const secret = (process.env.SMOHIX_DIGEST_CRON_SECRET ?? process.env.ZENTRO_DIGEST_CRON_SECRET)?.trim();
  if (!secret) {
