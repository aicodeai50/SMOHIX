...
import { deliverQuarterlyObligationCommitteeDigest } from "@/lib/compliance/quarterly-obligation-committee-digest";
...
/** Cron: POST with Bearer SMOHIX_OBLIGATION_COMMITTEE_DIGEST_CRON_SECRET. Body: `{ orgId, force? }` */
export async function POST(req: NextRequest) {
  const secret = (process.env.SMOHIX_OBLIGATION_COMMITTEE_DIGEST_CRON_SECRET ?? process.env.ZENTRO_OBLIGATION_COMMITTEE_DIGEST_CRON_SECRET)?.trim();
  if (!secret) {
  ...
  const result = await deliverQuarterlyObligationCommitteeDigest(actorId, orgId, {
