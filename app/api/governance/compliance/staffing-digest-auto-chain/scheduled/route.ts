...
import { runStaffingDigestAutoChain } from "@/lib/compliance/staffing-digest-auto-chain";
...
/** Cron: POST with Bearer SMOHIX_STAFFING_DIGEST_AUTO_CHAIN_CRON_SECRET. Body: `{ orgId, force? }` */
export async function POST(req: NextRequest) {
  const secret = (process.env.SMOHIX_STAFFING_DIGEST_AUTO_CHAIN_CRON_SECRET ?? process.env.ZENTRO_STAFFING_DIGEST_AUTO_CHAIN_CRON_SECRET)?.trim();
  if (!secret) {
  ...
  const result = await runStaffingDigestAutoChain(actorId, orgId, {
