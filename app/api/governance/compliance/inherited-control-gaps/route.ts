...
import {
  ...
  inheritedControlCoverageGapsToCsv,
...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildInheritedControlCoverageGapPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build inherited control coverage gap report." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.inherited_control_coverage_gaps_exported",
    user_id: user.id,
    org_id: orgContext.orgId,
    details: {
      format,
      period_days: pack.periodDays,
      vendor_count: pack.vendorCount,
      gap_count: pack.totalGapCount,
      vendors_with_gaps: pack.vendorsWithGaps,
    },
  });
  ...
  if (format === "csv") {
    return new NextResponse(inheritedControlCoverageGapsToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="inherited-control-gaps-${stamp}.csv"`,
      },
    });
  }
  ...
  return NextResponse.json(pack, {
    ...
    headers: {
      ...OPERATIONAL_RESPONSE_HEADERS,
      "Content-Disposition": `attachment; filename="inherited-control-gaps-${stamp}.json"`,
    },
  });
}
