...
import { runStaffingOverdueRemindersForOrg } from "@/lib/compliance/staffing-action-overdue-reminders";
...
/** Cron: POST with Bearer SMOHIX_STAFFING_OVERDUE_REMINDER_CRON_SECRET. Body: `{ orgId, force? }` */
export async function POST(req: NextRequest) {
  const secret = (process.env.SMOHIX_STAFFING_OVERDUE_REMINDER_CRON_SECRET ?? process.env.ZENTRO_STAFFING_OVERDUE_REMINDER_CRON_SECRET)?.trim();
  if (!secret) {
  ...
  const result = await runStaffingOverdueRemindersForOrg(actorId, orgId, {
