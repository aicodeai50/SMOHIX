...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildStaffingOverdueRemindersPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build staffing overdue reminders." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.staffing_action_overdue_reminders_exported",
    user_id: user.id,
    org_id: orgContext.orgId,
    details: {
      format,
      overdue_count: pack.overdueItems.length,
      open_action_count: pack.openActionCount,
    },
  });
  ...
  if (format === "csv") {
    return new NextResponse(staffingOverdueRemindersToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="staffing-overdue-${stamp}.csv"`,
      },
    });
  }
