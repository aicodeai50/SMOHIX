...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildControlBenchmarkPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build benchmark pack." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.control_benchmark_exported",
    user_id: user.id,
    org_id: orgContext.orgId,
    details: {
      format,
      period_days: pack.periodDays,
      org_overall_percentile: pack.orgOverallPercentile,
      catalog_version: pack.benchmarkCatalogVersion,
    },
  });
  ...
  if (format === "csv") {
    return new NextResponse(controlBenchmarkToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="control-benchmark-${stamp}.csv"`,
      },
    });
  }
  ...
  return NextResponse.json(pack, {
    ...
    headers: {
      ...OPERATIONAL_RESPONSE_HEADERS,
      "Content-Disposition": `attachment; filename="control-benchmark-${stamp}.json"`,
    },
  });
}
