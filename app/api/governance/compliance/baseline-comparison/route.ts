...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildBaselineComparisonPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build baseline comparison." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.baseline_comparison_exported",
    user_id: user.id,
    org_id: orgContext.orgId,
    details: {
      format,
      period_days: pack.periodDays,
      framework_count: pack.frameworkCount,
      lowest_readiness: pack.lowestReadinessFramework,
    },
  });
