...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildComplianceRiskHeatmap(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build risk heatmap." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.compliance_risk_heatmap_exported",
    user_id: user.id,
    ...
    details: {
      ...
      overall_level: pack.overallLevel,
      hotspot_count: pack.hotspots.length,
    },
  });
