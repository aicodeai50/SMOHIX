...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildCrossStaffingCommitteeEscalationPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build cross-staffing committee escalation." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.cross_staffing_committee_escalation_exported",
    user_id: user.id,
    org_id: orgContext.orgId,
    details: {
      format,
      breach_count: pack.breachItems.length,
      escalation_eligible: pack.escalationEligible,
      rollup_open_count: pack.rollupDelivery?.openCount ?? null,
    },
  });
  ...
  if (format === "csv") {
    return new NextResponse(crossStaffingEscalationToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="cross-staffing-escalation-${stamp}.csv"`,
      },
    });
  }
