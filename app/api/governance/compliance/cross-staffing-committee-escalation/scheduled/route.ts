...
/** Cron: POST with Bearer SMOHIX_CROSS_STAFFING_COMMITTEE_ESCALATION_CRON_SECRET. Body: `{ orgId, force? }` */
export async function POST(req: NextRequest) {
  const secret = (process.env.SMOHIX_CROSS_STAFFING_COMMITTEE_ESCALATION_CRON_SECRET ?? process.env.ZENTRO_CROSS_STAFFING_COMMITTEE_ESCALATION_CRON_SECRET)?.trim();
  if (!secret) {
  ...
  const result = await deliverCrossStaffingCommitteeEscalation(actorId, orgId, {
