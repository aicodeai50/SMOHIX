...
import {
  buildObligationConsolidationPlaybookPack,
  obligationConsolidationPlaybookToCsv,
...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildObligationConsolidationPlaybookPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build obligation consolidation playbook." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.obligation_consolidation_playbook_exported",
    user_id: user.id,
    org_id: orgContext.orgId,
    details: {
      format,
      horizon_days: pack.horizonDays,
      workflow_count: pack.workflowCount,
      crossover_cluster_count: pack.crossoverClusterCount,
      tracked: pack.stats.tracked,
    },
  });
  ...
  if (format === "csv") {
    return new NextResponse(obligationConsolidationPlaybookToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="obligation-consolidation-${stamp}.csv"`,
      },
    });
  }
  ...
  return NextResponse.json(pack, {
    ...
    headers: {
      ...OPERATIONAL_RESPONSE_HEADERS,
      "Content-Disposition": `attachment; filename="obligation-consolidation-${stamp}.json"`,
    },
  });
}
