...
import { runObligationDensityAlertsForOrg } from "@/lib/compliance/obligation-density-alerting";
...
/** Cron: POST with Bearer SMOHIX_OBLIGATION_DENSITY_ALERT_CRON_SECRET. Body: `{ orgId, force? }` */
export async function POST(req: NextRequest) {
  const secret = (process.env.SMOHIX_OBLIGATION_DENSITY_ALERT_CRON_SECRET ?? process.env.ZENTRO_OBLIGATION_DENSITY_ALERT_CRON_SECRET)?.trim();
  if (!secret) {
  ...
  const result = await runObligationDensityAlertsForOrg(actorId, orgId, {
