...
import { deliverStaffingCompletionRollup } from "@/lib/compliance/staffing-completion-rollup";
...
/** Cron: POST with Bearer SMOHIX_STAFFING_COMPLETION_ROLLUP_CRON_SECRET. Body: `{ orgId, force? }` */
export async function POST(req: NextRequest) {
  const secret = (process.env.SMOHIX_STAFFING_COMPLETION_ROLLUP_CRON_SECRET ?? process.env.ZENTRO_STAFFING_COMPLETION_ROLLUP_CRON_SECRET)?.trim();
  if (!secret) {
  ...
  const result = await deliverStaffingCompletionRollup(actorId, orgId, {
