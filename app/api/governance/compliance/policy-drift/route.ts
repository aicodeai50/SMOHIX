...
export async function GET(req: NextRequest) {
  ...
  const pack = await buildPolicyDriftPack(user.id, {
  ...
  if (!pack) {
    return NextResponse.json(
      { error: "Could not build policy drift report." },
      { status: 500, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  await appendAuditEvent({
    event_type: "governance.policy_drift_exported",
    user_id: user.id,
  ...
  if (format === "csv") {
    return new NextResponse(policyDriftToCsv(pack), {
      status: 200,
      headers: {
        ...OPERATIONAL_RESPONSE_HEADERS,
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="policy-drift-${stamp}.csv"`,
      },
    });
  }
  ...
  return NextResponse.json(pack, {
    ...
    headers: {
      ...OPERATIONAL_RESPONSE_HEADERS,
      "Content-Disposition": `attachment; filename="policy-drift-${stamp}.json"`,
    },
  });
}
