...
import { listSlackAuditEntriesForCsvExport } from "@/lib/audit/data";
...
export async function GET(req: NextRequest) {
  ...
  const rows = await listSlackAuditEntriesForCsvExport(user.id, { sinceIso });
  ...
  const filename = `slack-audit-${windowNorm}-${stamp}.csv`;
