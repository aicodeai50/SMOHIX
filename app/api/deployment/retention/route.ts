...
export async function GET() {
  ...
  const retention = await getRetentionPolicyForOrg(orgContext.orgId, profile.deploymentTier);
  if (!retention) {
    return NextResponse.json(
      { error: "Retention policy unavailable." },
      { status: 404, headers: OPERATIONAL_RESPONSE_HEADERS },
    );
  }

  return NextResponse.json({ profile, retention }, { status: 200, headers: OPERATIONAL_RESPONSE_HEADERS });
}
