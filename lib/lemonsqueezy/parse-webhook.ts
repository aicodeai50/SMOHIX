...
/** Checkout custom data: pass `{ "smohix_user_id": "<auth uuid>" }` from your app. */
export function extractSmohixUserId(meta: LemonMeta | undefined): string | undefined {
  const raw =
    meta?.custom_data?.smohix_user_id ?? meta?.custom_data?.supabase_user_id;
